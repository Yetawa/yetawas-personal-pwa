...
function initDate(){
  // 不预填 today()：留空即由服务端取「最近已收盘交易日」。
  // 旧版预填今天会把「今天」传回服务端，开盘前/盘中就会拿到
  // 「标签是今天、数值回退到上一交易日收盘价」的错位结果；load() 成功后会回填真实日期。
...
async function load(){
  ...
  try{
    ...
    const r=await fetch(url); const d=await r.json();
    if(d.error){ throw new Error(d.error); }
    // 日期框留空时由服务端决定「最近已收盘交易日」，用返回值回填，
    // 让用户看到数据实际对应的日期（避免开盘前标签写今天、数值却是上一交易日收盘价）。
    const _rd=document.getElementById('rdate'); if(_rd && !_rd.value && d.date){ _rd.value=d.date; }
...
<div class="panel">
  <div class="field"><label>查询日期（留空=最近收盘日）</label><input id="rdate" type="date"></div>
...
function renderTopHist(){
  ...
  rows.forEach(function(r){
    r._cur=(r.lp!=null)?r.lp:((latest[r.code]&&latest[r.code].price)||null);
...
async function load(){
  ...
  try{
    ...
    const r=await fetch(url); const d=await r.json();
    if(d.error){ throw new Error(d.error); }
    // 日期框留空时由服务端决定「最近已收盘交易日」，此处用返回值回填，
    // 让用户看到数据实际对应的日期（避免开盘前标签写今天、数值却是上一交易日）。
    const _rd=document.getElementById('rdate'); if(_rd && !_rd.value && d.date){ _rd.value=d.date; }
...
setInterval(tickClock, 1000); tickClock();
// 日期框不预填今天：留空即由服务端取「最近已收盘交易日」。
// 旧版预填 today() 会把「今天」作为参数传回服务端，开盘前/盘中就会拿到
// 「标签是今天、数值是上一交易日」的错位结果；load() 成功后会回填真实数据日期。
load();
if('serviceWorker' in navigator){
  window.addEventListener('load',()=>{ navigator.serviceWorker.register('/sw.js').catch(err=>console.log('SW 注册失败：',err)); });
}

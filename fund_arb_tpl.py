...
<div class="topbar">
  ...
  <div class="topnav">
    ...
    <a class="navlink" href="/cb">可转债套利</a>
...
<div class="histbox" id="topHist">
  <h3>近 5 个交易日溢价套利入选（≥1.5%，点击表头排序）</h3>
...
function renderCbHist(){
  ...
  var numKeys=['price','stock_price','_cur','_scur','_schg'];
  ...
  var head=cbHistCell('date','入选日期')+cbHistCell('code','转债代码')+cbHistCell('name','转债名称')+cbHistCell('price','入选日转债价')+cbHistCell('_cur','最新转债价')+cbHistCell('stock_name','正股')+cbHistCell('stock_code','正股代码')+cbHistCell('stock_price','入选日正股价')+cbHistCell('_scur','最新正股价')+cbHistCell('_schg','正股累计涨跌');
  box.innerHTML='<table class="histtbl"><thead><tr>'+head+'</tr></thead><tbody>'+rows.map(function(r){
    var cls=(r._schg==null)?'':(r._schg>=0?'pos':'neg');
    return '<tr><td>'+escCb(r.date)+'</td><td>'+escCb(r.code)+'</td><td class="l">'+escCb(r.name)+'</td><td>'+fmt(r.price)+'</td><td>'+fmt(r._cur)+'</td><td class="l">'+escCb(r.stock_name)+'</td><td>'+escCb(r.stock_code)+'</td><td>'+fmt(r.stock_price)+'</td><td>'+fmt(r._scur)+'</td><td class="'+cls+'">'+fmtPctCb(r._schg)+'</td></tr>';
  }).join('')+'</tbody></table>';
}
function applyTheme(t){
  document.documentElement.setAttribute('data-theme', t);

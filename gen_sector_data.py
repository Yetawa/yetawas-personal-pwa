...
INDEX_SECIDS = {
    ...
    "1.930955": "高估值", "1.930957": "低估值",
    "1.930975": "高动量", "1.930959": "低动量",
}
INDEX_NAMES = ["上证指数", "深证成指", "创业板指", "沪深300", "科创50", "北证50"]
STYLE_NAMES = ["大盘成长", "大盘价值", "小盘成长", "小盘价值", "上证50", "中证1000", "中证500", "中证2000", "高估值", "低估值", "高动量", "低动量"]
...
# 国证风格指数(930955/957/975/959)与中证2000(932000)在东财 ulist 不稳，优先用主流源补齐。
# 注：腾讯 sz932000 / sh930xxx 在部分网络不可达，新浪对国证指数亦有时缺失；
# 此处尽力覆盖全部 13 个，本机(国内网络)通常比沙箱更易拉全。
_SINA_CODES = {
    "中证500": "sh000905", "中证1000": "sh000852", "沪深300": "sh000300",
    "上证50": "sh000016", "中证2000": "sh932000", "大盘成长": "sz399372",
    "大盘价值": "sz399373", "小盘成长": "sz399376", "小盘价值": "sz399377",
    "高估值": "sh930955", "低估值": "sh930957", "高动量": "sh930975", "低动量": "sh930959",
}
_TENCENT_CODES = {
    "中证500": "sh000905", "中证1000": "sh000852", "沪深300": "sh000300",
...
def _fetch_style_hist(n_days=10):
    ...
    if not raw:
        return {}
    # 合并为按日期的矩阵；逐风格独立——某风格缺失仅跳过该风格当日，
    # 不再因「一个指数拉不到」把整日历史丢弃（否则 style_hist 恒为空、曲线图全无）。
    all_dates = set()

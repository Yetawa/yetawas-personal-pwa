...
def _cb_fnum(x):
    """东财字段安全转 float；'-'/None/非法值返回 None（新债常返回 '-'）。"""
    if x is None:
        return None
    try:
        return float(x)
...
def cb_compute(items, progress=None):
    ...
    for it in items:
        ...
        try:
            ...
            price = _cb_fnum(it.get("f2"))          # 转债最新价，新债零成交时东财返回 '-'

            cv = _cb_fnum(it.get("f236"))           # 转股价值

            prem = _cb_fnum(it.get("f237"))         # 转股溢价率(%)，<0 即折价(套利空间)
            ...
            volume = _cb_fnum(it.get("f5"))         # 成交量(手)，新债缺失为 None

        except (TypeError, ValueError):

            continue

        # 转债市价缺失('-')时，用东财同源公式反推：prem%=(price/cv-1)*100 -> price=cv*(1+prem/100)
        if price is None and cv is not None and prem is not None:
            price = cv * (1.0 + prem / 100.0)
        if price is None or price <= 0 or cv is None or cv <= 0:
            if price is None:
                print("    [可转债] skip %s(%s): 转债价缺失且无转股价值/溢价率可反推" % (code, name))
            continue
        # 可转债已停牌（成交量为 0，注意与缺失 '-' 区分）→ 不推送
        if volume is not None and volume <= 0:
            suspended_already += 1
            continue
        # 未来 2 个交易日内将停牌 → 不推送（接口不可用时 soon 为空集，自动跳过）
        if code in soon:
            suspended_soon += 1
            continue
